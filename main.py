...
def start(message):
    ...
    bot.reply_to(
        message,
        "🚀 PRO MAX STREAM BOT ACTIVE\n\n⚡ Ready to download & stream"
    )

@bot.message_handler(commands=['addadmin'])
def add_admin(message):
    if message.chat.id != OWNER_ID:
        return bot.reply_to(message, "❌ Only owner can add admin")
...
def remove_admin(message):
    if message.chat.id != OWNER_ID:
        return bot.reply_to(message, "❌ Only owner can remove admin")

    try:
        uid = int(message.text.split()[1])
        if uid == OWNER_ID:
            return bot.reply_to(message, "❌ Cannot remove owner")
